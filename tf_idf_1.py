...
data = os.listdir(file_root+'/data_txt/data_txt3') 
N = len(data) + 1
data_dict = {}
stat_dict ={}
...
def tf_idf(data_dict, query):
	print('tfidf in')
	with open(file_root+'/tf_idf_parameters.pickle', 'rb') as f:
	...
	for i in range(result_np.shape[0]-1):
		...
		score = np.dot(result_np[i],y)/(normal_x*normal_y)
	...
	with open('final_result.pickle', 'wb') as f:
